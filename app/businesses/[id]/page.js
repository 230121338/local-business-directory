      ...
      <div className="mt-4 grid gap-8 lg:grid-cols-[1.4fr_1fr]">
        <div>
          <div className="relative h-64 sm:h-80 w-full rounded-stall overflow-hidden border-2 border-ink">
            ...
            />
            {business.featured && (
              <span className="absolute top-3 left-3 stamp-verified bg-marigold text-ink text-xs font-mono font-semibold uppercase tracking-wide px-2.5 py-1 rounded-stall border-2 border-ink">
                Featured listing
              </span>
            )}
          </div>
        ...
        <aside className="lg:sticky lg:top-24 h-fit">
          <div className="bg-white border-2 border-ink rounded-stall p-5 space-y-4">
            ...
            {business.website && (
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="block text-center text-sm text-clay hover:text-clay-dark"
              >
                Visit website ↗
              </a>
            )}
